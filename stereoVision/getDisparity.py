...
def getDisparity(left_img, right_img, patch_radius, min_disp, max_disp):
    ...
    # We are not using padding. We calculate disparity based on the patch.
    for row in range(patch_radius, h-patch_radius):
        for col in range(max_disp+patch_radius, w-patch_radius):
            ...
            # Transform the patches into vectors
            lpvec = left_patch.flatten()
            rsvecs = rsvecs.reshape([patch_size**2, max_disp - min_disp +1])

            ssds = cdist(lpvec[None, :], rsvecs.T, 'sqeuclidean').squeeze()
            ...
            if reject_outliers:
                # Scenario 1: if more than 2 d candidates have a similar score to the smallest one we remove ambigous matches
                # Scenario 2: The min ssd occure at dmin or dmax. This might indicate there is a local minimum outside of the provided disparity
                #             range. We prefer fewer false positives than many true positives.
